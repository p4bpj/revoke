...
export async function GET(req: NextRequest) {
  ...
  try {
    const sql = `
      ...
      WHERE lower(owner) = $1 AND approved = true
      ORDER BY block_number DESC, id DESC
    `
    const { rows } = await query<DbOperatorApproval>(sql, [owner])
    ...
    console.error('DB error fetching operator approvals', err)
